...
from schemas.Image import UserViewer, Image, ImageCreate
...
@router.get("/{id}/images", response_model=List[Image], status_code=status.HTTP_200_OK)
async def get_user_images(id: int, db: Session = Depends(get_db)):
    return UserService.get_user_images(db, id)


@router.put("/{id}/images", response_model=Image, status_code=status.HTTP_201_CREATED)
